...
def validate_basis_abs_max(basis_abs_max: float | None) -> None:
    if basis_abs_max is None:
        return
    if basis_abs_max <= 0:
        raise ValueError("basis_abs_max must be > 0 when provided")
...
def prepare_train_config(raw_config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    config.setdefault("basis_abs_max", None)
    ...
    config.setdefault("static_side_input_enabled", False)
    config.setdefault("static_side_fusion_mode", "add")
    ...
    validate_basis_abs_max(config["basis_abs_max"])
    ...
    if config["static_side_input_enabled"] and not config.get("ordered_indices_path"):
        raise ValueError(
            "ordered_indices_path is required when static_side_input_enabled=True"
        )
    if config["static_side_fusion_mode"] not in {"add"}:
        raise ValueError(
            "static_side_fusion_mode currently only supports 'add'; "
            "planned-but-unimplemented modes include 'gate', 'film', and 'concat'"
        )
    return config
...
__all__ = [
    ...
    "resolve_side_weights",
    "validate_action_basis_init",
    "validate_basis_abs_max",
    "validate_aux_supervision_config",
    "validate_label_supervision_config",
    "validate_optional_init_path",
    "validate_region_basis_size",
    "validate_side_semantic_config",
]
