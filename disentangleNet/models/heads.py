...
def build_static_side_encoder(input_dim: int, output_dim: int) -> nn.Sequential:
    """
    TODO(recovery): recover the historical static-side encoder if PhaseAB or
    later stages turn `static_side_input_enabled=True`. The current path keeps
    the helper available for interface compatibility only.
    """
