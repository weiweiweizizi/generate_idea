"""
blendshape 与 SVD 时间系数相关性分析 - win10-step10 TT数据

验证思路：
1. 使用 win10-step10 TT联合SVD 的前4个主模态（PC1-PC4）
2. 对每个 TT 患者：
   - 将其数据投影到 TT_joint 基上，得到时间系数
   - 加载对应的 blendshape 数据（10帧窗口均值 → 前后差分）
   - 计算时间系数与 blendshape 变化的相关性
3. 分析 PC 与 blendshape 语义的对应关系

blendshape取法：
- 每10帧作为一个窗口
- 计算这10帧内各blendshape的均值
...
DATA_ROOT = REPO_ROOT / "data" / "win10-step10"
BLENDSHAPE_ROOT = REPO_ROOT / "data" / "blendshape"
OUTPUT_ROOT = REPO_ROOT / "outputs" / "pilot_feasibility" / "blendshape" / "win10-step10"
N_COMPONENTS = 4  # 分析前4个主成分
RANDOM_STATE = 42
WINDOW_SIZE = 10  # win10-step10

OUTPUT_DIR = OUTPUT_ROOT / "blendshape_correlation_results_win10"
...
def compute_blendshape_diff(blendshape_df, window_size=10):
    """
...
def main():
    print("=" * 60)
    print("blendshape 与 SVD 时间系数相关性分析 - win10-step10 TT")
    print("=" * 60)
    ...
    results_file = OUTPUT_DIR / "blendshape_correlation_win10_tt_results.json"
    ...
    plt.suptitle(f'Mean Absolute Correlation: SVD Time Coef vs Blendshape Diff (win10-step10 TT, n={len(patient_valid_info)})', fontsize=12)
    plt.tight_layout()
    ...
    ax.set_title(f'Correlation Distribution: PC1 & PC2 Top Blendshapes (win10-step10 TT, n={len(patient_valid_info)})', fontsize=12)
