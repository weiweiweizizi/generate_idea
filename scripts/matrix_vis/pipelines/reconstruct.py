...
def truncate_observations(
    ...
    *,
    ...
    if max_observations is None or observation_table.shape[0] <= max_observations:
        return observation_table, None
    ordered = observation_table.assign(_abs_value=observation_table["value"].abs()).sort_values(
        by="_abs_value",
        ascending=False,
        kind="mergesort",
    )
    truncated = ordered.head(max_observations).drop(columns=["_abs_value"]).reset_index(drop=True)
    return truncated, {
        "raw_count": int(observation_table.shape[0]),
        "used_count": int(truncated.shape[0]),
    }


def load_ground_truth_axis_trajectory(
    *,
    mesh,
    projection,
    basis_source: Path | None,
...
def run_axis_reconstruction(
    ...
    full_observation_table = basis_to_observation_table(basis_observation).frame
    observation_table, truncation_info = truncate_observations(
        full_observation_table,
        max_observations=cfg.solver.max_observations,
    )
    ...
    if cfg.export.save_axis_plot:
        ...
        ground_truth = load_ground_truth_axis_trajectory(
            mesh=mesh,
